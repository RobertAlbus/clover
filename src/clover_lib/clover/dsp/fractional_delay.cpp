...
fdl_lagrange::fdl_lagrange(size_t buffer_length)
    : m_buffer(buffer_length), m_max_idx(float(buffer_length - 3)) {
}
...
float fdl_lagrange::at(float idx) {
    ...
    float interpolation = idx - std::floor(idx);
...
void fdl_lagrange::tick(float x) {
    m_buffer.tick(x);
}
...
fdl_sinc::fdl_sinc(std::shared_ptr<circular_buffer>& buffer, size_t kernel_size, float window_alpha)
    ...
    m_min_idx     = std::floor(static_cast<float>(kernel_size) / 2) - 1;
    m_max_idx     = static_cast<float>(buffer->length()) - 1 - std::ceil(static_cast<float>(kernel_size) / 2);
    m_start_index = 0;
}

void fdl_sinc::delay(float idx) {
    if (idx > m_max_idx) {
        idx = m_max_idx;
