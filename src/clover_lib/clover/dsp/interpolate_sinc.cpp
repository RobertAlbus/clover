...
sinc_kernel::sinc_kernel(size_t size, float hann_alpha, float interpolation_amount) {
...
void sinc_kernel::set_interpolation(float interpolation_amount) {
...
float& sinc_kernel::operator[](size_t idx) {
...
void hann_window_with_corner_control(std::vector<float>& out, float alpha) {
    ...
    for (auto n : std::views::iota(0, (size / 2))) {
        float window_value =
                alpha + (1 - alpha) * 0.5f * (1 - std::cos((2 * num::pi * (float)n) / ((float)size - 1)));
...
void sinc_function(std::vector<float>& out, float interpolation_amount) {
    ...
    for (auto i : std::views::iota(0, size)) {
        float x = (static_cast<float>(i) - (static_cast<float>(size) / 2)) + (1 - interpolation_amount);
