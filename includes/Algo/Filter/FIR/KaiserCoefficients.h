...
std::vector<T> kaiser_lpf_coefficients(T cutoff_hz, T transition_bw_hz,
                                       T sample_rate_hz, T stopband_atten_db) {

  // Compute the filter coefficients using the windowed sinc function
  std::vector<T> coeffs(num_taps);
  T norm_cutoff_freq =
      static_cast<T>(2.0) * static_cast<T>(M_PI) * cutoff_hz / sample_rate_hz;
  T sum = static_cast<T>(0);
  for (int n = 0; n < num_taps; ++n) {
    T x = static_cast<T>(n) - mid_point;
    coeffs[n] = (x == 0) ? window[n] * norm_cutoff_freq / static_cast<T>(M_PI)
                         : window[n] * std::sin(norm_cutoff_freq * x) /
                               (static_cast<T>(M_PI) * x);
...
template <FloatingPoint T>
std::vector<T> kaiser_hpf_coefficients(T cutoff_hz, T transition_bw_hz,
                                       T sample_rate_hz, T stopband_atten_db) {
  auto coeffs = kaiser_lpf_coefficients<T>(cutoff_hz, transition_bw_hz,
                                           sample_rate_hz, stopband_atten_db);
...
template <FloatingPoint T>
std::vector<T> kaiser_bpf_coefficients(T low_cutoff_hz, T high_cutoff_hz,
                                       T transition_bw_hz, T sample_rate_hz,
                                       T stopband_atten_db) {

  // Design low-pass and high-pass filters
  auto low_pass_coeffs = kaiser_lpf_coefficients<T>(
      low_cutoff_hz, transition_bw_hz, sample_rate_hz, stopband_atten_db);
  auto high_pass_coeffs = kaiser_hpf_coefficients<T>(
      high_cutoff_hz, transition_bw_hz, sample_rate_hz, stopband_atten_db);
...
template <FloatingPoint T>
std::vector<T> kaiser_notch_coefficients(T center_freq_hz, T notch_bw_hz,
                                         T transition_bw_hz, T sample_rate_hz,
                                         T stopband_atten_db) {
  ...
  auto band_pass_coeffs = kaiser_bpf_coefficients<T>(
      low_cutoff_hz, high_cutoff_hz, transition_bw_hz, sample_rate_hz,
      stopband_atten_db);
