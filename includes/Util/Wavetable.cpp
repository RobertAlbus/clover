...
std::vector<Sample> LerpTable(int outputSize,
...
std::vector<Sample> Sine(int size) {
  ...
  for (int i = 0; i < size; i++) {
    wavetable.emplace_back((float)sin(((double)i / (double)size) * M_PI * 2.));
  }
...
std::vector<Sample> Pulse(int size, float pulseWidth) {
  ...
  wavetable.reserve(size);
  for (int i = 0; i < size; i++) {
    wavetable.emplace_back(i < ((float)size * pulseWidth) ? 1. : -1.);
  }
...
std::vector<Sample> Saw(int size) {
  ...
  for (int i = 0; i < size; i++) {
    wavetable.emplace_back((float)1. - (i * increment));
  }
