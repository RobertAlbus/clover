...
class MapAdapter
    : public StatefulProcessor<__arityIn, __arityOut, MapAdapterSettings> {
public:
  MapAdapter() {}
  MapAdapter(MapAdapterSettings initialSettings)
      : StatefulProcessor<__arityIn, __arityOut, MapAdapterSettings>(
            initialSettings) {
    std::for_each(initialSettings.mappings.cbegin(),
                  initialSettings.mappings.cend(),
                  [=, this](Mapping m) { this->validateMapping(m); });
  }

  /// BEWARE: there are no safeguards in place to prevent inefficient mappings
  /// or overwriting previously registered mappings
  ...
  void validateMapping(int from, int to) {
    ...
    if (!isFromValid || !isToValid) {
      printf("\n\ninvalid mapping in MapAdapter<%i,%i>: %i to %i.", __arityIn,
             __arityOut, from, to);
      if (!isFromValid)
  ...
  void validateMapping(int from, int count, int to) {
    ...
    if (rangeExceedsInputArity || rangeExceedsOutputArity) {
      printf("\n\ninvalid range mapping in MapAdapter<%i,%i>", __arityIn,
             __arityOut);
      if (rangeExceedsInputArity)
        printf("\n  mapping range of size %i exceeds the capacity of the input "
               "frame.",
               count);
      if (rangeExceedsOutputArity)
        printf("\n  mapping range of size %i exceeds the capacity of the "
               "output frame.",
               count);
      exit(EXIT_FAILURE);
    }
