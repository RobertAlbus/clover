...
public:
  BadSVF(float __cutoff, float reso, int mode = 1, int oversamplingFactor = 4,
         float __sampleRate = SAMPLE_RATE)
      : Subgraph(), dt_prime(0), fb_prime(0), _mode(mode),
        _oversamplingFactor(oversamplingFactor), lp(0), bp(0), hp(0) {
    cutoff(__cutoff);
    q(reso);
    // fir >> blackHole;
  }
...
protected:
  ...
  void oversamplingFactor(int over) {
    _oversamplingFactor = over;
  } // TODO: update FIR
