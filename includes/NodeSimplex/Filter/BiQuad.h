...
template <size_t __arity> class BiQuad : public Node<__arity, __arity> {
public:
  ...
  void set(float f, float q) { setFunction(f, q); }

  void freq(float frequency) { setFunction(frequency, reso_); }

  void reso(float Q) { setFunction(freq_, Q); }
  ...
  Frame<__arity> tick(Frame<__arity> input) {
    ...
    for (int i = 0; i < __arity; i++) {
      lastFrame_[i] =
          b_[0] * inputs_[0][i] + b_[1] * inputs_[1][i] + b_[2] * inputs_[2][i];
  ...
  */
  ...
  //! Set the filter coefficients for a high-pass with cutoff frequency \e fc
  //! (in Hz) and Q-factor \e Q.
  /*!
  ...
  */
  ...
  //! Set the filter coefficients for a band-reject centered at \e fc (in Hz)
  //! with Q-factor \e Q.
  /*!
    This method determines the filter coefficients corresponding to a
...
protected:
  ...
  // filter types along with the feedback filter coefficients. Performs the
  // debug check for fc and Q-factor arguments.
