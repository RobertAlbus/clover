...
{
public:
  Avoid(AvoidSettings initialSettings) : StatefulProcessor<__arity, __arity, AvoidSettings>(initialSettings)
  {

  }

  Avoid(float minimum, float maximum) : StatefulProcessor<__arity, __arity, AvoidSettings>()
  {
  ...
  void min(float minimum) 
  {
    ...
    this->settings.current.center = findCenter(minimum, this->settings.current.max);
  }

  void max(float maximum) 
  {
    this->settings.current.max = maximum;
    this->settings.current.center = findCenter(this->settings.current.min, maximum);
  }
